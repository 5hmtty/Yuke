# Yuke（语键）隐私政策（Privacy Policy）

最后更新：2026-09-18

Yuke（语键）是一款语音命令工具：**所有识别都在你的电脑上完成**。我们用最直白的方式说明数据处理方式：
...
6. **卸载与备份**：你的录音、发音与配置都在数据目录中（位置可在"关于"弹窗查看）。删除程序目录**不会**删除它们；卸载前如需转移，直接备份该数据目录即可。
...
**反作弊提醒**：个别游戏的反作弊系统可能限制或拦截模拟键盘输入，使用前请确认目标游戏的用户协议；Yuke 仅发送标准键盘事件，不读取内存、不注入任何进程。
...
6. **Uninstall & backup**: your recordings, phrases and settings live in the data folder (path shown in the About dialog). Deleting the app folder does **not** delete them — back up that folder to migrate.
